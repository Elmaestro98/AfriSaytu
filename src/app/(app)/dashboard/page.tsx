...
export default async function DashboardPage() {
    ...
    if (error instanceof SessionError && error.code === "MEMBER_DISABLED") {
      return (
        <AccessMessage
          ...
          text="Votre accès a été désactivé. Contactez le responsable de l'entreprise."
        />
      );
    }
    if (error instanceof SessionError && error.code === "NOT_A_MEMBER") {
      return (
        <AccessMessage
          ...
          text="Vous n'avez pas d'invitation valide pour cette entreprise. Demandez au responsable de vous inviter."
        />
      );
    }
    throw error;
  ...
  const [organization, today, recent, daily] = await Promise.all([
    ...
    can("transaction:view")
      ? listRecentOperations(ctx, 6, now)
      : Promise.resolve([]),
  ...
  return (
    <div className="flex flex-1 flex-col">
      <AppHeader
        title={organization?.name ?? "AfriSaytu"}
        subtitle={roleLabel(ctx.actor.role)}
      />

      <main className={cn(PAGE, "gap-6")}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col gap-2">
            <h1 className="font-heading text-3xl font-extrabold lg:text-4xl">
              Bonjour {firstName}
            </h1>
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              ...
              {today.openSince && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-accent px-2.5 py-0.5 text-xs font-semibold text-accent-foreground">
                  ...
                  />
                  Journée ouverte depuis{" "}
                  {formatDayLabel(today.openSince, now).toLowerCase()}{" "}
                  {formatTime(today.openSince)}
                </span>
              )}
            </div>
          </div>
        ...
        <section
          aria-label={
            ctx.actor.role === "AGENT" ? "Votre journée" : "Aujourd'hui"
          }
          className="grid gap-3 sm:grid-cols-2 lg:gap-4 xl:grid-cols-4"
        >
          <KpiCard
            label="Volume du jour"
            value={formatFCFA(today.volume)}
            icon={TrendingUp}
            tone="primary"
          ...
          {daily.rows.length > 0 ? (
            <KpiCard
              label="Commissions"
              value={`+${formatFCFA(today.commission + (isAgent ? 0 : daily.total))}`}
              icon={Coins}
            >
            ...
            <KpiCard
              label="Commissions"
              value={`+${formatFCFA(today.commission)}`}
              icon={Coins}
          ...
          >
            <span className="text-xs text-muted-foreground">
              {today.deposits} dépôt{today.deposits > 1 ? "s" : ""} ·{" "}
              {today.withdrawals} retrait{today.withdrawals > 1 ? "s" : ""}
            </span>
          </KpiCard>
          <KpiCard
            label="Trésorerie"
            value={formatFCFA(today.treasury.uv + today.treasury.cash)}
            icon={Landmark}
          >
            <span className="text-xs text-muted-foreground tabular-nums">
              UV {formatAmount(today.treasury.uv)} · Espèces{" "}
              {formatAmount(today.treasury.cash)}
            </span>
          </KpiCard>
        </section>

        {daily.rows.length > 0 && (
          <DailyCommissionsPanel
            daily={daily}
            showBranch={today.showBranch}
            forAgent={isAgent}
          />
        )}
          ...
          <div className="flex flex-col gap-3 lg:hidden">
            {can("transaction:view") && (
              <SettingsLink
                ...
                description={
                  ctx.actor.role === "AGENT"
                    ? "Vos commissions jour, semaine, mois"
                    : "Commissions par opérateur, type et agent"
                }
              />
            )}
            {ctx.actor.role !== "AGENT" && (
